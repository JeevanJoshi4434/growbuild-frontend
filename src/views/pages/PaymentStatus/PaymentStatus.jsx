...
const PaymentStatus = () => {
  ...
  const [leftPay, setLeftPay] = useState(null);
  ...
  const deletemaster= async(id)=>{
    ...
    const grabStatus = async()=>{
        const res = await axios.get(`${process.env.REACT_APP_PORT}/api/get/buyer/${buyerMaster.Project}/${buyerMaster.Building}/${buyerMaster.floor}/${buyerMaster.unit}/${buyerMaster.payment_stage}`);
        if(res.data?.status===404){
          setPayStatus("Not Found!");
        }else{
          const total = buyerMaster.price - res.data?.price;
          setLeftPay(res.data?.price);
          if(total <= 0) {
        ...
        >
          ...
          <div className="col-md-6 col-12 mb-2">
            ...
            {buyerMaster?.payment_stage !== null ?
              <button className="btn btn-primary" type="button" onClick={() =>{ fetchOwner(buyerMaster?.payment_stage); grabStatus();}}>Fetch Stage</button> :
          ...
          <div className="col-md-3 col-12 mb-2">
            ...
            {payStatus!=null ? <p className="text-alternate">{payStatus}</p> : <p>Select Stage first</p>}
          </div>
          <div className="col-md-3 col-12 mb-2">
            <p className="text-alternate">Status</p>
            {payStatus!=null ? <p className="text-alternate">{leftPay}</p> : <p>Select Stage first</p>}
          </div>
